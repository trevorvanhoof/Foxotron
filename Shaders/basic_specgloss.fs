...
void main(void)
{
  ...
  vec4 diffuse_alpha = sample_colormap( map_diffuse, out_texcoord );
  vec3 diffusemap = diffuse_alpha.xyz * diffuse_alpha.w; // premultiplied alpha
  ambient *= diffuse_alpha.w; // let's also apply alpha on the ambient lighting
  ...
  frag_color = vec4( pow( color * exposure, vec3(1. / 2.2) ), diffuse_alpha.w );
}
