...
namespace Renderer
{
  ...
  void Shader::SetTexture(const char* szTextureName, Texture* tex)
  {
    ...
    if (location != -1)
    {
      ...
      switch (tex->mType)
      {
      case TEXTURETYPE::TEXTURETYPE_1D: glBindTexture(GL_TEXTURE_1D, ((Texture*)tex)->mGLTextureID); break;
      case TEXTURETYPE::TEXTURETYPE_2D: glBindTexture(GL_TEXTURE_2D, ((Texture*)tex)->mGLTextureID); break;
      }
    }
  }
  ...
  Texture* CreateRGBA8TextureFromFile(const char* szFilename, const bool _loadAsSRGB /*= false*/)
  {
    ...
    tex->mWidth = width;
    tex->mHeight = height;
    tex->mType = TEXTURETYPE::TEXTURETYPE_2D;
  ...
  Texture* CreateRG32FTextureFromRawFile(const char* szFilename, int width, int height)
  {
    ...
    tex->mWidth = width;
    tex->mHeight = height;
    tex->mType = Renderer::TEXTURETYPE::TEXTURETYPE_2D;
    tex->mFilename = szFilename;
    tex->mGLTextureID = glTexId;
    tex->mGLTextureUnit = textureUnit++;
    return tex;
